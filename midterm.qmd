...
**Answer**: Before running it, I expect that the rectangle will probably not act in the way that we want it to. I see now that the rectangle actually goes over the line and so now we can't actually see anything past 2011. I anticipated something like this would happen because if you're making the whole graph before adding the background, then it layers on top of what was previously created. This leads to situations like these. The code for graphs needs to act like a cake where you layer tha elements in the right order so that you can see the top as the end result in the way that you want.
...
**Answer**: Since the p-value is less than $2.2^{-16}$, we know that the p-value is extremely small. Without having out $\alpha$ significance level explicitly defined, we can assume that $\alpha = 0.05$. Our p-value is much smaller than $\alpha$, so we can reject the null-hypothesis in this case. If $H_0$: The variables are independent, then therefore we have evidence to reject this hypothesis, and evidence suggests that the variables are not, in fact, independent.
...
SMDE_age_gender <-
  ...
  ggplot(aes(x = Year, y = Percent, group = Demographic)) +
    ...
    scale_x_continuous(breaks = seq(2006, 2018, by=1),
                       labels = seq(2006, 2018, by=1),
                       limits = c(2006, 2018)) +
    labs(title = "Treatment for Sevre Major Depressive Episode\namong Persons Aged 12 to 17",
         subtitle = "By Demographic Characteristics, Percentages, 2006-2018") +
    facet_wrap(~ subgroup) +
    ocs_theme()
...
# Saving our graph as PNG
save(SMDE_age_gender, file =here::here("plots", "SMDE_age_gender.rda"))
png(here::here("plots", "SMDE_age_gender.png"))
while (!is.null(dev.list()))  dev.off()
...
save(SMDE_race, file =here::here("plots", "SMDE_race.rda"))
png(here::here("plots", "SMDE_race.png"))
while (!is.null(dev.list()))  dev.off()
...
change_TMDES <- max_change(counts, "Age", "Treatment_for_Severe_Major_Depressive_Episode", Number)
...
pct_TMDES <- max_change(percents, "Age", "Treatment_for_Severe_Major_Depressive Episode", Percent)
...
pct_TMDE %<>% 
  full_join(max_val(percents, "Age", "Treatment", Percent), 
            by = "Demographic")
pct_TMDES %<>% 
  full_join(max_val(percents, "Age", "Treatment_for_Severe_Major_Depressive Episode", Percent), 
         by = "Demographic")
